                ...
                while ($row = mysqli_fetch_assoc($res)) : ?>
                    <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-9 gap-1 text-lg md:text-sm bg-white/60 hover:bg-teal-50 transition-all duration-150">
                        ...
                        <div class="px-1 md:px-2 py-1">
                            <button onclick="openModal(this, <?php echo htmlspecialchars($row['id']); ?>)" class="flex items-center gap-1 bg-teal-500 text-white px-3 py-1 rounded shadow hover:bg-teal-700 transition-all duration-150"><i class="fas fa-edit"></i> Edit</button>
                        </div>
                        <div class="px-1 md:px-2 py-1">
                            <form method="POST">
                                <button type="submit" name="delete" value="<?php echo htmlspecialchars($row['id']); ?>" class="flex items-center gap-1 bg-red-500 text-white px-3 py-1 rounded shadow hover:bg-red-700 transition-all duration-150"><i class="fas fa-trash"></i> Delete</button>
                            </form>
                        </div>
                    </div>
                <?php endwhile; ?>
            ...
            while ($row = mysqli_fetch_assoc($res)) : ?>
                <div class="bg-white/80 rounded-xl shadow p-4 flex flex-col gap-2">
                    ...
                    <div class="flex gap-2 mt-2">
                        ...
                        <form method="POST">
                            <button type="submit" name="delete" value="<?php echo htmlspecialchars($row['id']); ?>" class="flex items-center gap-1 bg-red-500 text-white px-3 py-1 rounded shadow hover:bg-red-700 transition-all duration-150 text-xs"><i class="fas fa-trash"></i> Delete</button>
                        </form>
                    </div>
                </div>
            <?php endwhile; ?>
...
<form method="POST" id="editForm" enctype="multipart/form-data">
    <div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white/90 p-4 rounded-xl shadow-2xl w-full max-w-xs transition-transform duration-200 hover:scale-105">
