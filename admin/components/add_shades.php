...
<div class="max-w-lg mx-auto bg-white/70 backdrop-blur-lg border border-teal-200 shadow-2xl rounded-2xl p-8 mt-8 transition-transform duration-200 hover:scale-105 hover:shadow-teal-200/60">
    ...
    <form id="responsiveForm" class="space-y-6" method="POST" enctype="multipart/form-data" action="insert_product_shades.php">
        ...
        <div class="relative mb-6">
            ...
            <select name="product_dropdown" id="product_dropdown"
                class="w-full p-3 pl-12 pr-10 rounded-xl border-2 border-teal-400 bg-white/70 shadow-lg focus:outline-none focus:ring-2 focus:ring-teal-400 text-lg font-semibold text-gray-800 transition-all duration-200 appearance-none"
                required>
                <option value="">-- Select Product --</option>
                <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                    <option class="rounded-xl border-2 border-teal-400 bg-white/70 shadow-lg focus:outline-none focus:ring-2 focus:ring-teal-400 transition-all duration-200 appearance-none" value="<?= $row['P_Id'] ?>"><?= $row['P_Name'] ?></option>
                <?php endwhile; ?>
            </select>
        ...
        <div id="imageBoxes" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <?php for ($i = 1; $i <= 4; $i++) : ?>
                <div class="bg-white/80 border-2 border-teal-100 p-4 rounded-xl flex flex-col items-center gap-2 transition-transform duration-200 hover:scale-105">
