                ...
                while ($row = mysqli_fetch_row($res)) : ?>
                    <div class="grid grid-cols-6 md:grid-cols-8 text-sm md:text-base bg-white/60 hover:bg-teal-50 transition-all duration-150">
                        ...
                        <div class="px-2 md:px-4 py-2">
                            <button onclick="openModal(this, <?php echo $row[0]; ?>)" class="flex items-center gap-1 bg-teal-500 text-white px-3 py-1 rounded shadow hover:bg-teal-700 transition-all duration-150"><i class="fas fa-edit"></i> Edit</button>
                        </div>
                        <div class="px-2 md:px-4 py-2">
                            <form method="POST">
                                <button type="submit" name="delete" value="<?php echo $row[0]; ?>" class="flex items-center gap-1 bg-red-500 text-white px-3 py-1 rounded shadow hover:bg-red-700 transition-all duration-150"><i class="fas fa-trash"></i> Delete</button>
                            </form>
                        </div>
                    </div>
                <?php endwhile; ?>
            ...
            while ($row = mysqli_fetch_row($res)) : ?>
                <div class="bg-white/80 rounded-xl shadow p-4 flex flex-col gap-2">
                    ...
                    <div class="flex gap-2 mt-2">
                        <button onclick="openModal(this, <?php echo $row[0]; ?>)" class="flex items-center gap-1 bg-teal-500 text-white px-3 py-1 rounded shadow hover:bg-teal-700 transition-all duration-150 text-xs"><i class="fas fa-edit"></i> Edit</button>
                        <form method="POST">
                            <button type="submit" name="delete" value="<?php echo $row[0]; ?>" class="flex items-center gap-1 bg-red-500 text-white px-3 py-1 rounded shadow hover:bg-red-700 transition-all duration-150 text-xs"><i class="fas fa-trash"></i> Delete</button>
                        </form>
                    </div>
                </div>
            <?php endwhile; ?>
