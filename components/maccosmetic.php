...
$sel = "SELECT * FROM product p INNER JOIN product_shades ps ON p.P_Id = ps.product_id WHERE p.P_Category = 'Mac' ORDER BY p.P_Id DESC;";
$res = mysqli_query($conn, $sel);
?>

<!-- Hero/Banner Section -->
<section class="relative h-72 flex items-center justify-center bg-cover bg-center mb-8" style="background-image: url('../Images/brands/MAC-Logo.png');">
    <div class="absolute inset-0 bg-black opacity-60"></div>
    <div class="absolute inset-0 bg-gradient-to-br from-gray-700/30 via-transparent to-gray-900/40 animate-gradient-move"></div>
    <div class="relative z-10 text-center">
        <h1 class="text-5xl md:text-6xl font-extrabold text-white drop-shadow-lg mb-2 animate-fade-in">MAC Cosmetics</h1>
        <p class="text-lg text-gray-200 animate-fade-in delay-200 mb-4">Discover iconic MAC products for every look, from bold to natural. Unleash your creativity with professional-grade makeup.</p>
        <a href="#Products" class="inline-block bg-black text-white text-lg font-semibold px-8 py-3 rounded-lg shadow-lg hover:bg-gray-800 transition-all duration-200 animate-fade-in-up">Shop All <i class="fa fa-arrow-down ml-2"></i></a>
    </div>
</section>

<section class="Products px-4 md:px-36 pt-6 pb-16 bg-gradient-to-br from-white via-gray-50 to-gray-100 animate-fade-in-up" id="Products">
    ...
    <div class="inline-block relative mb-8 animate-fade-in-up">
        <button id="filterDropdownBtn" class="border border-black px-4 py-2 rounded-md text-black font-bold flex items-center space-x-2 bg-white shadow hover:bg-gray-100 transition-all duration-200">
        ...
        <div id="filterDropdown" class="hidden absolute bg-white border border-gray-300 shadow-2xl w-64 p-4 mt-2 rounded-xl z-50 animate-fade-in-up">
            ...
            <div class="flex justify-between mt-4">
                <button id="clearFilter" class="border border-black px-4 py-2 text-black rounded-md hover:bg-gray-200 hover:text-black transition-all duration-200 shadow">CLEAR</button>
                <button id="applyFilter" class="bg-black text-white px-4 py-2 rounded-md hover:bg-gray-800 transition-all duration-200 shadow">APPLY</button>
            </div>
        </div>
    </div>
    ...
    <div class="box-container grid grid-cols-1 sm:grid-cols-2 gap-9 lg:grid-cols-4 md:grid-cols-4 animate-fade-in-up" data-id="1">
        <?php while ($row = mysqli_fetch_assoc($res)) : ?>
            <div class="product" data-id="<?php echo $row['P_Id']; ?>" data-type="<?php echo strtolower($row['P_Product']); ?>">
                <div class="box p-6 bg-white overflow-hidden shadow-lg rounded-2xl border border-opacity-20 text-center relative hover:shadow-2xl hover:scale-105 hover:border-black border transition-all duration-300 cursor-pointer animate-fade-in-up">
                    ...
                    <p class="text-black text-3xl md:text-4xl mt-2 font-bold">₹<?php echo $row['P_Price']; ?></p>
                </div>
            </div>
        <?php endwhile; ?>
    </div>
    <div id="noProductMessage" class="flex flex-col items-center justify-center text-center text-black text-xl hidden mt-12 animate-fade-in-up">
        <img src="../Images/empty-box.png" alt="No Products" class="w-32 h-32 mx-auto mb-4 opacity-70">
        <span>No Product Added Yet</span>
    </div>

    <!-- Back to Top Button -->
    <button id="backToTopBtn" class="fixed bottom-8 right-8 z-50 bg-black text-white p-4 rounded-full shadow-lg hover:bg-gray-800 transition-all duration-200 hidden"><i class="fa fa-arrow-up"></i></button>
</section>
