...
$sel = "SELECT * FROM product p INNER JOIN product_shades ps ON p.P_Id = ps.product_id WHERE p.P_Category = 'HUda Beauty' ORDER BY p.P_Id DESC;";
$res = mysqli_query($conn, $sel);
?>

<!-- Hero/Banner Section -->
<section class="relative h-72 flex items-center justify-center bg-cover bg-center mb-8" style="background-image: url('../Images/hudabeauty/huda-beauty-logo.jpg');">
    ...
    <div class="relative z-10 text-center">
        <h1 class="text-5xl md:text-6xl font-extrabold text-white drop-shadow-lg mb-2 animate-fade-in">Huda Beauty</h1>
        <p class="text-lg text-gray-200 animate-fade-in delay-200 mb-4">Unleash your inner beauty with the latest Huda Beauty products, curated for bold, glamorous looks.</p>
...
<section class="Products px-4 md:px-36 pt-6 pb-16 bg-gradient-to-br from-white via-gray-50 to-pink-50 animate-fade-in-up" id="Products">
    ...
    <div class="inline-block relative mb-8 animate-fade-in-up">
        <button id="filterDropdownBtn" class="border border-black px-4 py-2 rounded-md text-black font-bold flex items-center space-x-2 bg-white shadow hover:bg-pink-50 transition-all duration-200">
    ...
    <div class="box-container grid grid-cols-1 sm:grid-cols-2 gap-9 lg:grid-cols-4 md:grid-cols-4 animate-fade-in-up" data-id="1">
        <?php while ($row = mysqli_fetch_assoc($res)) : ?>
            <div class="product" data-id="<?php echo $row['P_Id']; ?>" data-type="<?php echo strtolower($row['P_Product']); ?>">
                <div class="box p-6 bg-white overflow-hidden shadow-lg rounded-2xl border border-opacity-20 text-center relative hover:shadow-2xl hover:scale-105 hover:border-pink-400 border transition-all duration-300 cursor-pointer animate-fade-in-up">
