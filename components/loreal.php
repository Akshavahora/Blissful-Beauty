...
$sel = "SELECT * FROM product p INNER JOIN product_shades ps ON p.P_Id = ps.product_id WHERE p.P_Category = 'Loreal' ORDER BY p.P_Id DESC;";
$res = mysqli_query($conn, $sel);
?>

<!-- Hero/Banner Section -->
<section class="relative h-72 flex items-center justify-center bg-cover bg-center mb-8" style="background-image: url('../Images/brands/Loreal-Paris-Logo.png');">
    <div class="absolute inset-0 bg-black opacity-60"></div>
    <div class="absolute inset-0 bg-gradient-to-br from-yellow-400/30 via-transparent to-yellow-600/40 animate-gradient-move"></div>
    <div class="relative z-10 text-center">
        <h1 class="text-5xl md:text-6xl font-extrabold text-white drop-shadow-lg mb-2 animate-fade-in">L'Oreal Paris</h1>
        <p class="text-lg text-gray-200 animate-fade-in delay-200 mb-4">Experience the elegance of L'Oreal Paris – innovative beauty for every style and every day.</p>
        <a href="#Products" class="inline-block bg-yellow-600 text-white text-lg font-semibold px-8 py-3 rounded-lg shadow-lg hover:bg-yellow-700 transition-all duration-200 animate-fade-in-up">Shop All <i class="fa fa-arrow-down ml-2"></i></a>
    </div>
</section>

<section class="Products px-4 md:px-36 pt-6 pb-16 bg-gradient-to-br from-white via-yellow-50 to-yellow-100 animate-fade-in-up" id="Products">
    ...
    <div class="inline-block relative mb-8 animate-fade-in-up">
        <button id="filterDropdownBtn" class="border border-yellow-600 px-4 py-2 rounded-md text-yellow-700 font-bold flex items-center space-x-2 bg-white shadow hover:bg-yellow-50 transition-all duration-200">
            <i class="fa fa-filter text-yellow-600"></i>
        ...
        <div id="filterDropdown" class="hidden absolute bg-white border border-yellow-200 shadow-2xl w-64 p-4 mt-2 rounded-xl z-50 animate-fade-in-up">
            ...
            <div class="flex justify-between mt-4">
                <button id="clearFilter" class="border border-yellow-600 px-4 py-2 text-yellow-700 rounded-md hover:bg-yellow-100 hover:text-yellow-800 transition-all duration-200 shadow">CLEAR</button>
                <button id="applyFilter" class="bg-yellow-600 text-white px-4 py-2 rounded-md hover:bg-yellow-700 transition-all duration-200 shadow">APPLY</button>
            </div>
        </div>
    </div>
    ...
    <div id="noProductMessage" class="flex flex-col items-center justify-center text-center text-yellow-700 text-xl hidden mt-12 animate-fade-in-up">
        <img src="../Images/empty-box.png" alt="No Products" class="w-32 h-32 mx-auto mb-4 opacity-70">
        <span>No Product Added Yet</span>
    </div>

    <!-- Back to Top Button -->
    <button id="backToTopBtn" class="fixed bottom-8 right-8 z-50 bg-yellow-600 text-white p-4 rounded-full shadow-lg hover:bg-yellow-700 transition-all duration-200 hidden"><i class="fa fa-arrow-up"></i></button>
</section>
