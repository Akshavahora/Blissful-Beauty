...
if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['place_order'])) {
    ...
    foreach ($cart as $item) {
        if (isset($item['id']) && isset($item['quantity'])) {
            ...
            if (isset($item['shade_product_id'])) {
                $shade_id = intval($item['shade_product_id']);
                $check_stock_query = "SELECT quantity FROM stock WHERE product_id = $product_id AND shade_id = $shade_id";
                $stock_result = $conn->query($check_stock_query);
                $stock_row = $stock_result ? $stock_result->fetch_assoc() : null;
                $available_stock = $stock_row ? intval($stock_row['quantity']) : 0;
    ...
    if (!$all_in_stock) {
        echo "<script>alert('Not enough stock for one or more selected shades. Please adjust your cart.'); window.location.href = 'checkout.php';</script>";
        exit;
    }

    // 2. Only now insert address, payment, order, order items, and update stock
    ...
    if ($conn->query($order_query)) {
        ...
        foreach ($cart as $item) {
            if (isset($item['id']) && isset($item['price']) && isset($item['quantity'])) {
                ...
                if (isset($item['shade_product_id'])) {
                    $shade_id = intval($item['shade_product_id']);
                    $update_stock_query = "UPDATE stock SET quantity = quantity - $quantity WHERE product_id = $product_id AND shade_id = $shade_id";
